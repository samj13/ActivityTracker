...
The learning task is to classify intensity of activities based on measurements such as:
-Heartbeat
-Accelorometer
-Magnometer
There are 3 intensities to be classified:
Light(1): Lying, Sitting, Standing, Ironing
Moderate(2): Walking, Cycling, Nordic Walking, Descending Stairs, Vacuuming
Vigorous(3): Running, Ascending Stairs, Rope Jumping
...
def testI(tree, points, I):
    ...
    return mismatchesPerI(tree,points)[I]/tree.locCounts[I]
...
def makeTree(points, curNode, maxDepth, gainCutOff, gainPast):
    ...
    intensities = [2,3,4]
    #Create intensity counts, the intensity counts are still stored in activityCounts
    activityCounts = []
    for intensity in intensities:
        activityCounts.append([ex[0] for ex in points].count(intensity))
    ...
    for i in range(1,46):
        ...
        #Sort points? Make a list with only those examples that contain that feature
        ...
        for j in range(0,pointsL):
            ...
            belowCounts = []
            for intensity in intensities:
                belowCounts.append([ex[0] for ex in sortedPoints[0:j]].count(intensity))
            gain1 = percentBelow*(eHelper(belowCounts,j))

            #Calculate Entropy Above Split which includes those that do not have feature                       
    ...
    if sum(1 for i in activityCounts if i) < 2 or maxGain < gainCutOff or maxGain==gainPast or depth(curNode)>=maxDepth: #Very good chance this will have to be changed
        curNode.makeLeaf(intensities[activityCounts.index(max(activityCounts))], activityCounts)
        #Return to parent
        return curNode
    ...
    #For convention, left node are less than the split and right nodes are greater
    #Make left node
...
train = open('C:/Users/Sam/Documents/Cornell University/Fall 12 Semester/CS 4780/Final Project/iPhone Data/Final Files/iid/AkshayiPhoneData66.train')
trainpoints = []
for line in train:
...
#Test Mappings
#The data is coming in line by line as specified by [intensityID featureID:featureVal....]
test = open('C:/Users/Sam/Documents/Cornell University/Fall 12 Semester/CS 4780/Final Project/iPhone Data/Final Files/iid/AkshayiPhoneData33.test')
testpoints = []
for line in test:
    splits = re.split(':|\s',line)
    tempVec = [float("infinity")]*141 #One index has activityID the other 140 are the features
...
#Save tree
pickle.dump(tree, open('savedTree.txt','wb'))
